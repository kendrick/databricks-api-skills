...
- **PATCH** `.../{name}` -- body: `comment`, `owner`, `properties`, `isolation_mode`. Requires owner or metastore admin
...
- **Optional:** `comment`, `storage_root`, `properties`
...
- **PATCH** `.../{full_name}` -- body: `comment`, `owner`, `properties`, `name` (rename)
